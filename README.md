...
单次 15 秒 H3 latent 需要约 362 帧，序列长度约为 5 秒门禁的三倍，注意力显存可能按平方增长，不能在 4×16GB 上直接放开。网关现在接受 `duration: 15, segments: 3`：每段仍是已经通过容量门禁的 124 帧，分别完成真实 TP4、VAE 和音频解码，再用 FFmpeg 生成 360 帧、24 FPS、15.0 秒成片。三段使用相邻 seed，段间连续性不是单次长序列级别，可能出现跳切；这条路径已完成本机合成媒体的帧数/时长/可解码性测试，远端 GPU 端到端验收仍需单独记录。
