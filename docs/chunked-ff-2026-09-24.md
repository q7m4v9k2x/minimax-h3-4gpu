...
| 640×1152、124 帧、4096 行 | 分块通过 | 13.99 / 14.70 GiB（15.02 / 15.78 GB） | 288.52 / 289.10 s | finite |
| 640×1152、124 帧、8192 行 | 分块通过 | 13.99 / 14.70 GiB（15.02 / 15.78 GB） | 287.73 / 288.30 s | finite |
...
因此 `h3_ff_chunk_rows=4096/8192` 只应作为高分辨率实验开关；在当前 640×1152 测试中 8192 行比 4096 行略快且峰值不变，但生产发布前必须用真实 conditioning、固定验收样例和最终 VAE 输出重新做画质回归。需要逐位复现时保持 `h3_ff_chunk_rows=0`。
