...
本报告把一次请求拆成可以独立计时和验收的阶段。当前工作站上的真实报告只覆盖预计算 conditioning 到 DiT latent；没有 H3 视频/音频 VAE 权重，因此 VAE、媒体编码和 API 响应阶段必须保持“未测量”，不能用 DiT 时间代替。
...
| VAE | 当前未测 | 远端没有 H3 视频/音频 VAE 权重；不能宣称端到端时延 | 补齐权重后按 tile/time chunk 单独计时，DiT 完成后释放激活 |
| 编码、落盘 | 当前未测 | 直接写最终路径会造成刷新后历史丢失或半文件 | 临时文件、fsync、原子 rename；记录文件 SHA256 |
