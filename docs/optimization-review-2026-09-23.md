...
- DiT 峰值约 12.48 GiB allocated（13.40 GB；reserved 13.33 GiB/14.32 GB），20 步约 124.16 秒，即约 6.18 秒/步。
...
- 同一 640×1152 条件把 `h3_ff_chunk_rows` 从 4096 调到 8192 后，DiT 约 287.73 秒、pipeline 约 288.30 秒，峰值不变；因此当前高分辨率实验配置优先采用 8192。
...
严格 A/B 还显示，关闭 `h3_finite_check` 的收益大于在 480×864 上启用分块：无分块、无 finite 检查为 119.218 秒；4096 行分块为 121.106 秒，但显存从 12.48/13.33 GiB 降到 12.22/12.81 GiB。分块的作用是显存边界，不是普遍提速；8192 行只在 640×1152 实测为更快的实验档。
